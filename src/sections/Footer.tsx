      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-12">
          ...
          <div className="md:col-span-2">
            <KelkaLogo className="h-16 mb-4" variant="light" />
            <p className="text-white/60 text-sm leading-relaxed max-w-xs">
              A Kelka desenvolve soluções inovadoras de higiene e bem-estar animal. Cuidamos do seu pet como se fosse nosso.
            </p>
            {/* Social icons */}
            <div className="flex gap-4 mt-6">
              {[
                ...
                <a
                  key={s.label}
                  href={s.href}
                  className="w-9 h-9 rounded-xl bg-white/10 hover:bg-emerald-400/20 hover:border-emerald-400/40 border border-white/10 flex items-center justify-center text-white/70 hover:text-white transition-all duration-200"
                  aria-label={s.label}
                >
                  {s.icon}
                </a>
          ...
          <div>
            <h4 className="font-black text-sm uppercase tracking-widest text-emerald-400 mb-4">Empresa</h4>
            <ul className="space-y-3">
              {["Sobre a Kelka", "Sustentabilidade", "Blog", "Trabalhe Conosco", "Contato"].map((l) => (
                <li key={l}>
                  <a href="#" className="text-white/60 hover:text-emerald-400 text-sm transition-colors">{l}</a>
                </li>
              ))}
            </ul>
